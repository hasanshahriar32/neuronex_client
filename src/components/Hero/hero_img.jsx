...
export const HeroImage = () => {
    ...
    useEffect(() => {
        ...
        if (!isComponentMounted || !inView || isLoading) return;
        ...
        renderLine(randomNumberBetween(800, 1500));
        return () => {
            if (timeoutRef.current) clearTimeout(timeoutRef.current);
        };
    }, [inView, setLines, isComponentMounted, isLoading]);
    useEffect(() => {
        setIsLoading(false);
    }, []);

    return (
        <div className="mt-[10.8rem] [perspective:2000px]">
            <div ref={ref} className={classNames(
                ...
                <svg
                    ...
                    )} width="100%" viewBox="0 0 1499 778" fill="none" >
                ...
                <img
                    className={classNames("relative z-10 transition-opacity delay-[680ms]",
                        inView ? "opacity-100" : "opacity-0")} src={img} alt="Hero image" />
            </div>
        </div>
    );
};
