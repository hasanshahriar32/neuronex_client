...
const buttonClasses = (variant, size, highlight) => {
    ...
    if (variant === "primary") {
        classes += ` bg-primary-gradient hover:text-shadow hover:shadow-primary transition-[shadow,text-shadow] ${highlight && "ml-2"}`;
    } else if (variant === "secondary") {
        classes += ` text-off-white bg-white bg-opacity-10 border border-transparent-white backdrop-filter-[12px] hover:bg-opacity-20 transition-colors ease-in ${highlight && "bg-transparent-white rounded-full px-2"} ${highlight && highlight + ":last-child"} ml-2 ${highlight && highlight + ":last-child"} -mr-2 ${highlight && highlight + ":first-child"} -ml-2 ${highlight && highlight + ":first-child"} mr-2`;
    }
